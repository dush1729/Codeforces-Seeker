...
object AppDatabaseMigrations {
    ...
    val MIGRATION_13_14 = object : Migration(13, 14) {
        override fun migrate(connection: SQLiteConnection) {
            // Re-create problemset_problem with all columns
            connection.execSQL("DROP TABLE IF EXISTS `problemset_problem`")
            connection.execSQL("""
                CREATE TABLE IF NOT EXISTS `problemset_problem` (
                    `contestId` INTEGER NOT NULL,
                    `problemsetName` TEXT,
            ...
            """)
            connection.execSQL(
                "CREATE INDEX IF NOT EXISTS index_problemset_problem_rating ON problemset_problem(rating)"
            )
            connection.execSQL(
                "CREATE INDEX IF NOT EXISTS index_problemset_problem_name ON problemset_problem(name)"
            )

            // Re-create solved_problem table
            connection.execSQL("DROP TABLE IF EXISTS `solved_problem`")
            connection.execSQL("""
                CREATE TABLE IF NOT EXISTS `solved_problem` (
                    `handle` TEXT NOT NULL,
                    `contestId` INTEGER NOT NULL,
    ...
    val MIGRATION_14_15 = object : Migration(14, 15) {
        override fun migrate(connection: SQLiteConnection) {
            connection.execSQL("""
                CREATE TABLE IF NOT EXISTS `rated_user` (
                    `handle` TEXT NOT NULL,
